...
setImmediate(async () => {
  ...
  app.use(
    ...
    async (req, res, next) => {
      if ((req.user as any).user_id == null) {
      // if (req.cookies.id_token.user_id == null) {
        // tslint:disable-next-line:no-console
        console.log(
          [
            ' xx ',
      ...
      const resCurrentUserAccount = await knex('user_account')
        .select(['id'])
        .where({ id: (req.user as any).user_id });
      ...
      if (resCurrentUserAccount[0] == null) {
        ...
        console.log(
          [
            ...
            JSON.stringify((req.user as any).user_id),
  ...
  app.get('/download-data/:exportId', async (req, res) => {
    ...
    if ((req.user as any).user_id !== exportItem.owner_id) {
  ...
  app.get('*', async (req, res, next) => {
    ...
    try {
      ...
      const location = store.getState().location as any;
