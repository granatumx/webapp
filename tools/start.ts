...
function createCompilationPromise(name, compiler: webpack.Compiler, config) {
  return new Promise((resolve, reject) => {
    let timeStart = new Date();
    compiler.hooks.compile.tap('start.ts', () => {
      timeStart = new Date();
      console.info(`[${format(timeStart)}] Compiling '${name}'...`);
    });
    compiler.hooks.done.tap('start.ts', (stats: webpack.Stats) => {
...
async function start() {
  ...
  let appPromise;
  let appPromiseResolve = () => null;
  let appPromiseIsResolved = true;
  serverCompiler.hooks.compile.tap('weirdResolve', () => {
  ...
  function checkForUpdate(fromUpdate = null) {
